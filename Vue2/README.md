...
- 状态管理：当应用状态复杂时，需要使用Vuex进行状态管理。

### vuex工作原理

![Vuex原理图](./imgs/Vuex原理图.png)

- 工作流程
  - 将需要共享的数据，存储store
  - 组件可以从store中获取获取数据
  - 组件可以通过dispatch方法，触发action执行异步操作
  - action可以通过commit方法，提交mutation修改store中的数据(mutation修改的数据会被DevTools工具记录，方便调试)
  - 最新数据会被同步到依赖该数据的所有组件中，触发组件重新渲染
- 特殊说明
  - mutation只能同步修改数据，不能异步操作
  - action可以异步操作，但是不能直接修改state中的数据，必须通过commit方法提交mutation来修改数据
  - 如果无异步操作，可以在组件中直接通过commit方法提交mutation来修改数据

### 搭建vuex环境

- 安装vuex模块（vue2需要安装3.x版本）
```bash
