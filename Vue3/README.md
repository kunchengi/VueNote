...
- props
  - 父传子: 父组件向子组件传递数据和方法
  - 子传父：子组件通过调用父组件传递的方法，向父组件传递数据
- $attrs
  - 本质是props
  - 祖先传后代
    - 祖先组件向后代组件传递数据和方法
    - 父组件给子组件传递的属性，如果未在props中定义，就会被收集到$attrs中
    - 可以通过props层次传递$attrs，在需要的后代组件中使用
  - 后代传祖先
    - 后代组件可以通过调用祖先组件传递的方法，向祖先组件传递数据
- 自定义事件
  - 子传父
  - 子组件通过触发自定义事件，向父组件传递数据
- v-model
  - 子传父
  - 本质是自定义事件的语法糖
  - 子组件通过update:modelValue触发自定义事件，实现子组件向父组件传递数据
- mitt
  - 任意组件间的通信
- $refs
  - 父传子：父组件通过访问子组件的实例，调用子组件的方法向子组件传递数据
  - 子传父：父组件通过访问子组件的实例，获取子组件对外提供的数据
- $parent
  - 子传父：子组件通过访问父组件的实例，调用父组件的方法向父组件传递数据
  - 父传子：子组件通过访问父组件的实例，获取父组件对外提供的数据
- private和inject
  - 祖先传后代：祖先组件可以使用provide提供数据给后代组件，后代组件可以使用inject注入数据
  - 后代传祖先：后代组件可以通过调用祖先组件传递的方法，向祖先组件传递数据
- 插槽
...
- 可以将被teleport包裹的标签渲染到指定的标签内，而不是默认的位置
```html
  <!-- 模态框组件 -->
  <template>
      <!-- 由于组件默认渲染到父元素内,如果没有写teleport，modal的位置是相对于父元素的 -->
      <!-- 可以将模态框渲染到body标签内,如果写了teleport，modal的位置是相对于to选择的元素 -->
      <!-- to的内容时class选择器的字符串 -->
      <teleport to="body">
          <div class="modal">
